...
public abstract class Algorithm<A,S,N extends Node<A,S,N>> implements Iterable<N> {
    ...
    public final class SearchResult {
        ...
        public List<List<S>> getOptimalPaths(Context ctx, String destino) {
            List<List<S>> paths = new ArrayList<List<S>>(goalNodes.size());
            for(N goalNode : goalNodes){
                paths.add(recoverStatePath(goalNode, ctx, destino));
            }
        ...
        public String toString() {
            ...
            for(N goalNode : goalNodes){
                ...
                builder.append("\t").append(recoverStatePath(goalNode, null, "-1").toString()).append(ls);
     ...
     */
    public static <S, N extends Node<?,S,N>>  List<S> recoverStatePath(N node, Context ctx, String destino){
        ...
        for(N n : node.path()){
            ...
            String bairroAtual = n.state().toString();
            List<ItinerarioPartidaDestino> a = null;
            List<ItinerarioPartidaDestino> b = null;

            if(db != null){
                a = db.itinerarioDAO().executarComando(new SimpleSQLiteQuery("SELECT id FROM tpr2 WHERE idBairroPartida = '"
                        +bairroAtual+"' AND idBairroDestino = '"+destino+"'"));
            }

            String bairroAnterior = "";

            if(states.size() > 1){
                bairroAnterior = states.get(states.size()-2).toString();
            }

            if(db != null && !bairroAnterior.isEmpty()){
                b = db.itinerarioDAO().executarComando(new SimpleSQLiteQuery("SELECT id FROM tpr2 WHERE idBairroPartida = '"
                        +bairroAnterior+"' AND idBairroDestino = '"+bairroAtual+"'"));
            }
