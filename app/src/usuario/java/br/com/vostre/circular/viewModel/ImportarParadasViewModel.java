...
public class ImportarParadasViewModel extends AndroidViewModel {
    ...
    public ImportarParadasViewModel(Application app){
        super(app);
...
//        servicos = appDatabase.servicoDAO().listarTodosAtivos();
    ...
    public void buscarBairroParada(Parada parada){
        new buscarBairroAsyncTask(appDatabase).doInBackground(parada);
    }

    private static class buscarBairroAsyncTask extends AsyncTask<Parada, Void, Void> {

        private AppDatabase db;

        buscarBairroAsyncTask(AppDatabase appDatabase) {
            db = appDatabase;
        }
        ...
        protected Void doInBackground(final Parada... params) {
            ...
            Parada pbi = params[0];
            Bairro b = db.bairroDAO().encontrarPorNomeECidade(pbi.getBairro(), pbi.getCep());

            if(b != null){
                pbi.setBairro(b.getId());
            }
    ...
    public void buscarEnderecoGoogle(final Parada parada){
        Retrofit retrofit = new Retrofit.Builder()
                .addConverterFactory(ScalarsConverterFactory.create())
        ...
        Call<String> call = api.carregaEnderecoGoogle(parada.getLatitude()+","+parada.getLongitude(),
                getApplication().getApplicationContext().getString(R.string.key_google));
        call.enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                ...
                try {
                    ...
                    if(obj != null){
                        JSONArray dadosRua = obj.getJSONArray("results");

                        JSONObject enderecoCompleto = dadosRua.getJSONObject(0);

                        JSONArray endereco = enderecoCompleto.getJSONArray("address_components");

                        int totalDados = endereco.length();
                        ...
                        for(int i = 0; i < totalDados; i++){
                            ...
                            JSONArray tipos = (JSONArray) ((JSONObject) endereco.get(i)).get("types");

                            switch((String) tipos.get(0)){
                                case "street_number":
                                    numero = ((JSONObject) endereco.get(i)).getInt("long_name");
                                    break;
                                case "route":
                                    rua = ((JSONObject) endereco.get(i)).getString("long_name");
                                    break;
                                case "political":
                                    bairro = ((JSONObject) endereco.get(i)).getString("long_name");
                                    break;
                                case "administrative_area_level_2":
                                    cidade = ((JSONObject) endereco.get(i)).getString("long_name");
                                    break;
                                case "administrative_area_level_1":
                                    estado = ((JSONObject) endereco.get(i)).getString("long_name");
                                    break;
                            }

                        }

                        System.out.println(numero+", "+rua+", "+bairro+","+cidade);

                        if(numero > 0){
                            parada.setNome("Próximo ao "+numero);
                        }
                        ...
                        if(!cidade.isEmpty()){
                            parada.setCep(cidade); //usando apenas temporariamente, ja que nao busca cep nessa funcao
                        }

                        //buscarBairroParada(parada);
                        retorno.setValue(2);
