...
public class MapaViewModel extends AndroidViewModel {
    ...
    public void setParada(final ParadaBairro parada, boolean isFeriado) {
        ...
        final SimpleSQLiteQuery query = new SimpleSQLiteQuery("SELECT i.*, e.nome AS 'nomeEmpresa', " +
                ...
                "FROM itinerario i INNER JOIN empresa e ON e.id = i.empresa " +
                "WHERE i.id IN (SELECT pi.itinerario FROM parada_itinerario pi INNER JOIN parada p ON p.id = pi.parada " +
                "WHERE p.id = '"+parada.getParada().getId()+"' " +
                "AND p.id <> (SELECT pp.id FROM parada pp WHERE pp.id = i.paradaFinal) AND proximoHorario IS NOT NULL AND pi.ativo = 1) " +
                " " +
